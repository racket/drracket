...
(let ([f
  ...
  (let ([h '(case-lambda
	     [(x y) 'two]
	     [(x y z w) 'four])])
    (error-test (list h) arity?)
    (error-test (list* h '(1)) arity?)
    (error-test (list* h '(1 2 3)) arity?)
    (error-test (list* h '(1 2 3 4 5 6)) arity?)))
...
(error-test '(set! unbound-variable 5) exn:variable?)
...
(error-test '(case 0 ((0) =>))  exn:variable?)
(syntax-test '(and . 1))
(syntax-test '(and 1 . 2))
(syntax-test '(or . 1))
(syntax-test '(or 1 . 2))
...
(test-values '(3 84) (lambda () (letrec*-values ([(x y) (values 3 84)]) (values x y))))
...
(define (error-test-let expr)
  (syntax-test (cons 'let expr))
  (syntax-test (cons 'let (cons 'name expr)))
  (syntax-test (cons 'let* expr))
  (syntax-test (cons 'letrec expr))
  (syntax-test (cons 'letrec* expr)))
(error-test-let 'x)
(error-test-let '(x))
(error-test-let '(()))
...
(define (do-error-test-let-values expr syntax-test)
  (syntax-test (cons 'let-values expr))
  (syntax-test (cons 'let*-values expr))
  (syntax-test (cons 'letrec-values expr))
  (syntax-test (cons 'letrec*-values expr)))
...
(let ([wrap (lambda (body)
	      (syntax-test `(let () ,@body))
	      (syntax-test `(let () (begin ,@body))))])
...
(define (qq-test e)
  (syntax-test e )
  (syntax-test (list 'quasiquote e))
  (syntax-test (list 'quasiquote e))
  (syntax-test (list 'quasiquote (list 'quasiquote e)))
  (syntax-test (list 'quasiquote (list 'quasiquote (list 'unquote e))))
  (syntax-test (list 'quasiquote (list 'quasiquote (list 'unquote-splicing e)))))
(qq-test '(unquote))
...
(if (zero? (car (cons 0 0))) (define ed-t3 3) (define ed-t3 -3))
(test 3 'begin-define ed-t3)
...
(error-test '(fluid-let ([x 10]) (error 'bad)) exn:user?)
...
(error-test '(fluid-let ([undefined-variable 0]) 8) exn:variable?)
...
(error-test '(parameterize ([error-print-width 777]) (error 'bad)) exn:user?)
