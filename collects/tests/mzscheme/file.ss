...
(error-test '(open-input-file "badfile") exn:i/o:filesystem:file?)
...
(let ([conflict? exn:application:mode-conflict?]
      [modes '(binary text)]
...
(error-test '(let ([c (make-custodian)])
		 ...
		 (display 'hi p)))
	    exn:i/o:port-closed?)
(error-test '(open-output-file "tmp4" 'error) exn:i/o:filesystem:file-exists?)
...
(define (test-format format format-name)
  ...
  (error-test `(,@format-name "~x" 'a) exn:application:fprintf:argument-type?)
  (error-test `(,@format-name "~x" 4.0) exn:application:fprintf:argument-type?)
  (error-test `(,@format-name "~x" 5+4.0i) exn:application:fprintf:argument-type?))
