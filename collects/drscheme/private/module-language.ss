               ...
               (let ([body (car bodies)]
                     [loop-with-ids
                      (lambda (ids)
				       ...
				       ;; is macro-generated, and if the defined name also 
				       ;; originates from a module, then it's hidden to anything
				       ;; that wasn't generated by the same macro invocation. This
				       ;; hiding is exposed by the fact that `syntax-e' of the
				       ;; identifier returns a different name than `identifier-binding'.
				       (lambda (id)
