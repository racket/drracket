...
@MACRO PROGRESS = if (x3 < x4 || x5 < x3) WITH_VAR_STACK(scheme_arg_mismatch(METHODNAME("slider%","initialization"), "minimum, value, and maximum must be increasing; given minimum: ", p[4]));
@MACRO NOZEROX[p] = if (x<p> <= 0) x<p> = 1;

@CREATOR (wxPanel!,wxFunction/bCallback/ubCallback/cCallback//spCallback/nopush,nstring,int,int,int,int,int=-1,int=-1,SYM[sliderStyle]=wxHORIZONTAL,string="slider"); : : ubCallbackSetup/PROGRESS|NOZEROX[6]//ubCallbackCreatorFinish
