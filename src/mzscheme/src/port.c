...
#ifdef USE_FD_PORTS
static void flush_fd(Scheme_Output_Port *op)
{
  Scheme_FD *fop = (Scheme_FD *)op->port_data;

  if (fop->bufcount) {
    long len;
    len = write(fop->fd, fop->buffer, fop->bufcount);
    if (len != fop->bufcount) {
      /* Uh-oh */
    }
    fop->bufcount = 0;
  ...
  *s = sizeof(Scheme_Input_Port);
  ...
  if (ip->sub_type == file_input_port_type)
    ...
    *s += (sizeof(Scheme_Indexed_String)
    ...
    if (!scheme_lookup_in_table(ht, (const char *)ip->port_data)) {
