...
/* MATTHEW: NO_AUTO_STACK_PUSH */
#if defined(ASM_CLEAR_CODE) && !defined(THREADS) && !defined(NO_AUTO_STACK_PUSH)
...
ptr_t arg;
{
    ...
    /* MATTHEW: NO_AUTO_STACK_PUSH */
#   if defined(THREADS) || defined(NO_AUTO_STACK_PUSH)
        word dummy[CLEAR_SIZE];;
#   else
    	register word limit;
#   endif
...
#   endif
   ...
   /* MATTHEW: In case we use near data for 68k Mac, this array is declared FAR */
#ifdef __MC68K__
