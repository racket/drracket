...
(define simple-settings make-simple-settings)
  ...
  (define (create-executable-gui parent program-filename show-type show-base)
    ...
    (define (browse-callback)
      ...
      (define-values (base name _)
        (if (path-string? ftf)
            (split-path ftf)
            (values (current-directory) "" #f)))
      (define mzscheme? (currently-mzscheme-binary?))
      (define mode (current-mode))
      (define filename
        (put-executable/defaults dlg
                                 base
                                 name
                                 mode
                                 (not mzscheme?)
    ...
    (define (check-filename)
      ...
      (define mode (current-mode))
      (define-values (extension style filters) (mode->put-file-extension+style+filters mode mred?))
      (cond
        [(string=? "" filename-str)
         (message-box (string-constant drscheme)
  ...
  (define (put-executable/defaults parent default-dir default-name mode mred? title)
    ...
    (define dir?
      (case mode
        ...
        [(stand-alone) (embedding-executable-is-directory? mred?)]
        [(distribution) #f]))
    (define users-name
      (if dir?
          (get-directory title parent default-dir style)
          (put-file title parent default-dir default-name extension style filters)))
    (and users-name
         (users-name-ok? mode extension parent users-name)
         (or (not dir?)
             (gui-utils:get-choice (format (string-constant warning-directory-will-be-replaced)
                                           users-name)
                                   (string-constant yes)
                                   (string-constant no)
